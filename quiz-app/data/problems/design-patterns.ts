...
export const designPatternProblems: Problem[] = [
  {
    ...
    conceptExplanation:
      'Compound Component(복합 컴포넌트) 패턴은 부모 컴포넌트가 내부 상태를 Context로 관리하고, 자식 컴포넌트들이 그 상태를 공유하며 협력하는 React 디자인 패턴입니다. 사용자는 자식 컴포넌트들을 원하는 위치에 자유롭게 배치할 수 있어 높은 유연성을 제공합니다. Radix UI나 Headless UI 같은 UI 라이브러리에서 널리 채택하고 있습니다.',
...
<Accordion>
  ...
  },
  {
    ...
    conceptExplanation:
      'Render Props 패턴은 컴포넌트가 직접 UI를 렌더링하는 대신, 렌더링 함수를 props(또는 children)로 받아 호출하여 UI를 외부에서 제어하는 패턴입니다. 상태 관리 로직은 컴포넌트 안에 두고, 그 상태로 무엇을 그릴지는 사용자가 결정합니다. React Hooks 등장 이후 대부분의 경우 커스텀 훅으로 대체되었지만, 렌더링 위치 제어가 필요한 경우 여전히 유용합니다.',
...
<DataLoader url="/api/users">
  ...
  },
  {
    ...
    conceptExplanation:
      'HOC(Higher-Order Component, 고차 컴포넌트)는 컴포넌트를 인자로 받아 새로운 컴포넌트를 반환하는 함수입니다. 인증, 로깅, 에러 처리처럼 여러 컴포넌트에 공통으로 필요한 횡단 관심사(cross-cutting concerns)를 재사용 가능한 형태로 분리합니다. 함수가 함수를 받아 새 함수를 반환하는 고차 함수(Higher-Order Function) 개념을 컴포넌트에 적용한 것입니다.',
...
// Hooks: 상태/사이드이펙트 로직 재사용 (대부분의 경우)`,
  ...
  },
  {
    ...
    conceptExplanation:
      '커스텀 훅(Custom Hook)은 React 내장 훅을 조합하여 특정 관심사의 상태와 사이드 이펙트 로직을 추출한 재사용 가능한 함수입니다. 이름이 반드시 use로 시작해야 React가 훅 규칙을 적용합니다. 여러 컴포넌트에서 동일한 로직이 반복된다면 커스텀 훅으로 추출하는 것이 권장됩니다.',
...
function useAsync<T>(fn: () => Promise<T>, deps: any[]) {
  ...
  },
  {
    ...
    conceptExplanation:
      'Container/Presenter(컨테이너/프레젠터) 패턴은 컴포넌트를 데이터와 비즈니스 로직을 담당하는 Container와 UI 렌더링만 담당하는 Presenter로 분리하는 디자인 패턴입니다. Presenter는 props만 받아 렌더링하므로 Storybook이나 단위 테스트에서 독립적으로 개발할 수 있습니다. React Hooks 도입 이후 Container 역할을 커스텀 훅이 대신하는 경우가 많아졌습니다.',
...
// 3. UserPage는 통합 테스트`,
  ...
  },
  {
    ...
    conceptExplanation:
      '프론트엔드 프로젝트의 폴더 구조 전략은 크게 기술 유형별(components/, hooks/, services/)과 기능(도메인)별(features/users/, features/posts/)로 나뉩니다. 기능별 구조는 관련 파일을 한 곳에 모아 응집도를 높이고, 기능 추가·수정·삭제 시 변경 범위를 최소화합니다. 프로젝트 규모가 커질수록 기능별 구조가 유지보수에 유리합니다.',
...
// 3. 기능별 독립 배포/테스트 가능`,
  ...
  },
  {
    ...
    conceptExplanation:
      'SOLID 원칙은 객체지향 설계의 5가지 원칙의 앞 글자를 딴 것으로, 유지보수하기 좋은 소프트웨어 설계를 위한 지침입니다. 그 중 SRP(Single Responsibility Principle, 단일 책임 원칙)는 하나의 모듈/클래스/컴포넌트가 하나의 변경 이유만 가져야 한다는 원칙입니다. React에서는 데이터 페칭, 상태 관리, UI 렌더링 각각을 별도 훅이나 컴포넌트로 분리하는 방식으로 적용합니다.',
    code: `// 리팩토링 전
...
// "변경 이유를 최소화하고, 재사용성을 높이자"는 방향성으로 이해하세요`,
  ...
  },
  {
    ...
    conceptExplanation:
      'Observer 패턴은 관찰 대상(Subject)의 상태가 변경될 때 그것을 구독하는 관찰자(Observer)들에게 자동으로 알림을 보내는 디자인 패턴입니다. Subject와 Observer가 느슨하게 결합되어 있어 Subject는 Observer가 누구인지 알 필요가 없습니다. DOM의 addEventListener, React의 useEffect 의존성 배열, 상태 관리 라이브러리의 구독 메커니즘 등이 이 패턴의 실제 적용 예시입니다.',
    options: [
      'for...of 루프',
