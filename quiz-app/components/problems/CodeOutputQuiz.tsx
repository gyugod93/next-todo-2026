...
export default function CodeOutputQuiz({ problem, onSubmit, initialAnswer, onRetry }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <p className="text-sm text-gray-400 mb-3">출력 결과는?</p>
        <div className="space-y-2">
          {problem.options!.map((option, index) => (
            <button
              ...
              disabled={submitted}
              className={`w-full text-left px-4 py-3 rounded-lg border transition-all duration-150 text-sm font-mono ${optionStyle(index)}`}
            >
              <span className="text-gray-500 mr-3 font-sans">
